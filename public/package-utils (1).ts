...
function addItemsToPackage(packageJava: string, items: string[]): { adjustPackage: string; isChanged: boolean } {
  ...
  for (const item of adjustItems) {
    ...
    /*nếu là object thì: findPackage(adjustPakage, regex(key))
     nếu có: addItemsToPackage(adjustPakage, value)
     nếu không: addItemsToPackage(adjustPakage, value)
    */
  }
...
export const addPackageToFile = (
  Contentfile: any,
  path: string[],
  keyList: string[],
  value: any,
  folderType: Constant.PathType.POM_FOLDER | Constant.PathType.GRADLE_FOLDER,
  isReplaceMatch: boolean = false
): string | undefined => {
  // Nếu value là một mảng, duyệt qua từng phần tử và thêm vào Contentfile
  if (Array.isArray(value)) {
    value.forEach((item) => {
      Contentfile = addPackageToFile(Contentfile, path, keyList, item, folderType, isReplaceMatch) ?? Contentfile;
    });
    return Contentfile;
  }

  // Nếu value là một object, duyệt qua từng cặp key-value và gọi đệ quy
  if (typeof value === 'object' && value !== null) {
    Object.entries(value).forEach(([key, item]) => {
      const newPath = [...path, key]; // Cập nhật đường dẫn với key hiện tại
      Contentfile = addPackageToFile(Contentfile, newPath, keyList, item, folderType, isReplaceMatch) ?? Contentfile;
    });
    return Contentfile;
  }

  if (typeof value === 'string') {
    if (folderType === Constant.PathType.POM_FOLDER) {
      Contentfile[0] = value;
      return Contentfile;
    } else if (folderType === Constant.PathType.GRADLE_FOLDER) {
      // check tồn tại hay chưa, nếu tồn tại thì thôi
      // chưa tồn tại thì push
    }
  }
  return undefined;
};
...
const getOrCreateNodeListByPath2 = (contentFile: any, path: string[]): [] => {
  return path.reduce((currentNode, key) => {
    if (Array.isArray(currentNode)) {
      currentNode = currentNode[0] ??= {};
    }
